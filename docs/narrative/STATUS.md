...
| Документы / надписи на стенах | `level1.json`–`level6.json`, спрайты `sprites/pickups/documents/` | done (карточки, записки, дневники и надписи раскрывают заключение героя, эксперименты и трансформацию) |
...
| Documents / artifacts     | `sprites/pickups/documents`, `sprites/pickups/artifacts` (пусто) | `inventory.ts`, HUD `#hudDocsValue`, `level1.json`–`level6.json` | partial (документы и их контент готовы; отдельные artifacts отсутствуют) |

## 12. Уровни

| Уровень         | Файл          | Соответствие нарративу                                                                                                                                                                                                                                                                                        | Статус                                                 |
| --------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| Палата          | `level1.json` | 56×42 ward, 5 этажей × 2 крыла: archive(gold-lock)/post, procedure/shower, ward A/day-room, ward B/storage, spawn-холл; разные door/window textures для архива, душевой, палат, ложных окон; husk×4 + orderly×2 ambush; hallucinations в shower/ward A; silence-burst в душевой; show_ending(clean) на выходе | done                                                   |
