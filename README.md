...
Интерактивный источник текста задаётся сущностью `message` (поддерживается и прежний
`note`) с полем `messageType`. В карте хранится только категория и внешний вид:
...
Взятие любого ключа всегда расходует одно сообщение из категории `note`. Поэтому
